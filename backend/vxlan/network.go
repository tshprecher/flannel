...
func (n *network) Run(ctx context.Context) {
	glog.Info("Watching for L3 misses")
	...
	for _, evt := range batch {
		...
		case subnet.EventAdded:
			...
			if evt.Lease.Attrs.BackendType != "vxlan" {
				glog.Warningf("Ignoring non-vxlan subnet: type=%v", evt.Lease.Attrs.BackendType)
				continue
			}

			var attrs vxlanLeaseAttrs
			if err := json.Unmarshal(evt.Lease.Attrs.BackendData, &attrs); err != nil {
				glog.Error("Error decoding subnet lease JSON: ", err)
				continue
			}
...
func (n *network) handleInitialSubnetEvents(batch []subnet.Event) error {
	glog.Infof("Handling initial subnet events")
	...
	for _, fdbEntry := range fdbTable {
		glog.Infof("fdb already populated with: %s %s ", fdbEntry.IP, fdbEntry.HardwareAddr)
	}
	...
	for j, marker := range fdbEntryMarker {
		if !marker && fdbTable[j].IP != nil {
			...
			if err != nil {
				glog.Error("Delete L2 failed: ", err)
			}
		}
	}
...
func (n *network) handleMiss(miss *netlink.Neigh) {
	...
	case len(miss.IP) == 0 && len(miss.HardwareAddr) == 0:
		glog.Info("Ignoring nil miss")

	case len(miss.HardwareAddr) == 0:
		n.handleL3Miss(miss)

	default:
		glog.Infof("Ignoring not a miss: %v, %v", miss.HardwareAddr, miss.IP)
	}
}

func (n *network) handleL3Miss(miss *netlink.Neigh) {
	glog.Infof("L3 miss: %v", miss.IP)

	rt := n.rts.findByNetwork(ip.FromIP(miss.IP))
	if rt == nil {
		glog.Infof("Route for %v not found", miss.IP)
		return
	}

	if err := n.dev.AddL3(neigh{IP: ip.FromIP(miss.IP), MAC: rt.vtepMAC}); err != nil {
		glog.Errorf("AddL3 failed: %v", err)
	} else {
		glog.Info("AddL3 succeeded")
	}
}
